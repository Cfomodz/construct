...
var OrbitControls = function ( object, domElement, scene ) {
  // jrh - begin
  ...
  function handleMouseWheel( event ) {
    ...
    // jrh - begin
    var cameraPosition = JSON.stringify({x: scope.object.position.x,
                                         y: scope.object.position.y,
                                         z: scope.object.position.z})
    localStorage.setItem('cameraPosition', cameraPosition)
    // jrh - end
  ...
  function handleKeyDown( event ) {
    ...
    switch ( event.keyCode ) {
      ...
      case scope.keys.W:
        if (event.shiftKey) {
          // console.log('+Z!')
          if (getInverseKinematics({x: position.x, y: position.y, z: position.z + 10})) {
          ...
          // console.log('+Y!')
          if (getInverseKinematics({x: position.x, y: position.y + 10, z: position.z})) {
            var tween = new TWEEN.Tween(position)
      ...
      case scope.keys.D:
        // console.log('+X!')
        if (getInverseKinematics({x: position.x + 10, y: position.y, z: position.z})) {
          var tween = new TWEEN.Tween(position)
  ...
  function onMouseMove( event ) {
    ...
    var target = JSON.stringify({x: scope.target.x,
                                 y: scope.target.y,
                                 z: scope.target.z})
